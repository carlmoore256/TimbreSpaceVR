...
public class GrainNoLOD : MonoBehaviour
{
    ...
    public void Initialize(GrainAudioFeatures features, GrainModelParameters modelParameters, Action<PlaybackEvent> onGrainActivated) {
        ...
        UpdateScale(modelParameters.ScaleFeature, 0.05f, 1f);
    ...
    public void UpdateScale(AudioFeature f, float multiplier=0.05f, float scaleExp=1f, float scaleMin=0)
    {
    ...
    IEnumerator PlayCoroutine(float duration = 1f) {
        ...
        float time = 0f;        
        while (time < duration) {
            sphereRenderer.material.color = Color.Lerp(sphereRenderer.material.color, targetColor, time/duration);
